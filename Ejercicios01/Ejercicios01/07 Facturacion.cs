...
namespace Ejercicios07
{
	...
	class Factura : IFactura
	{
		...
		public double ImporteTotal => items.Sum( p => p.Value * i.Precio( p.Key ) ); // items.Select( p => p.Value * i.Precio( p.Key ) ).Sum(); 
	...
	class DemoFacturaConInventario
	{
		static void Main( string[] args )
		{
			Console.WriteLine("* DEMO Facturación con control de Inventario *\n");
